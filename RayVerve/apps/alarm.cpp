...
static bool DrawToggle(Rectangle r,bool val){
    ...
    DrawRectangleRounded(r,0.45f,10,val?NEON_GREEN:BG_HOVER);
...
static void DrawTabs(int sw){
    ...
    for(int i=0;i<4;i++){
        ...
        DrawRectangleRounded(r,0.3f,8,act?BG_HOVER:BG_PANEL);
...
static void DrawWorldTab(Rectangle c,int sw){
    ...
    if(DrawButton({list.x+list.width-108,list.y+8,94,24},"+ Add City",BG_HOVER,NEON_CYAN,FONT_TINY)){addCityModal=true;citySearch="";searchFocused=true;}
...
int main(){
    if(!RequestResources(APP_NAME,RAM_MB,HDD_MB,PRIORITY_HIGH,0)){
        InitWindow(440,120,"Alarm - Denied");SetTargetFPS(30);
        double t=GetTime();
    ...
    InitWindow(WIN_W,WIN_H,"RayVerve Alarm");
